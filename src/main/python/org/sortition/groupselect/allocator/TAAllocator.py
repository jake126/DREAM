...
class TAAllocator:
    def __init__(self, tables, people, cats_cluster, cats_diverse, m_data, nallocations, cluster_tables, val_cluster,pareto_prob,n_swap_loops,progress_bar,previous_meetings,
no_cluster_agents):
	# 2025 new: previous_meetings moved to argument
        self.pareto_prob = pareto_prob
        self.tables = tables # TODO 2025 - what if this is list?
        ...
        self.manual_pids = []
	self.no_cluster_agents = no_cluster_agents
    ...
    # place manual PIDs TODO 2025: PV: I have commented this out, as it doesn't work with a list of tables. It would be an easy fix, and would essentially need to be integrated into the table-wise loop within "run" rather than being its own function as "template" and "seats" both depend on the number of tables. The difficulty here is that if we have 10 individuals, and we want person a b and c on table 1, that will work for rounds in which there are 4 or fewer tables but for 5+ tables there will be insufficient space. This problem gets more complex if we want to put d and e on table 2. We can discuss, but easier to drop for now
...
#            self.template[t][ss] = pid
    ...
    def run(self,progress_bar):
        ...
        for round_no in range(n_rounds):
            ...
            # NEW 2025: list of tables
	    if len(self.tables)>1:
	        no_tables = self.tables[round_no]
	    else:
		no_tables = self.tables
	    	    
	    # remove empty seats if assymetrical tables
            no_larger_tables = self.m_data%no_tables
	    seats = math.ceil(self.m_data/no_tables)
# NEW 2025: dynamic calculation of number of cluster tables
	    min_cluster_tables = math.ceil(self.no_cluster_agents/seats)
	    n_cluster_tables = min(min_cluster_tables + cluster_tables,tables)
            ...
            if no_larger_tables == 0:
                ...
                template = [[None for s in range(seats)] for r in range(no_larger_tables)]+[[None for s in range(seats-1)] for r in range(no_smaller_tables)]
        
	    meetings_previous_round = self.previous_meetings.copy()
            # run the algorithm for a single round of meetings
            round_assign_pre, round_assign_swap, meetings_pre = self.run_round(template,self.n_swap_loops, seats)
    ...
    def run_round(self, template, n_swap_loops, seats):
